...
from mcm.gcp_model_with_bp_treatment import GCPModel
from mcm.test.do_not_change_risk_factors_model_repository import DoNotChangeRiskFactorsModelRepository
...
class TestGCPModel(unittest.TestCase):
    ...
    def test_baseline_gcp(self):
        ...
        self.assertEqual(GCPModel().calc_linear_predictor(person=self._test_case_one, test=True),
        ...
        self.assertAlmostEqual(
            64.45419405-2.7905, GCPModel().calc_linear_predictor(person=self._test_case_one, test=True), places=1)
        ...
        self.assertAlmostEqual(
            50.01645213+1.3320, GCPModel().calc_linear_predictor(person=self._test_case_two, test=True), places=1)

        # this is for the the first black person in the spreadsheet (J150483aric)...comparator is GCP + no cohort effect (ARIC)
        self.assertAlmostEqual(42.99471241, GCPModel().calc_linear_predictor(
            person=self._test_case_three, test=True), places=1)

    def test_gcp_after_one_year(self):
        # this is for the first person in teh spreadsheet (1569nomas)...comparator is GCP + backing out the cohort effect (NOMAS)
        self._test_case_one.advance_year(DoNotChangeRiskFactorsModelRepository(), AlwaysNegativeOutcomeRepository())
        # account for the difference between the actual mean systolic change
        self.assertAlmostEqual(64.28862964-2.7905, GCPModel().calc_linear_predictor(person=self._test_case_one, test=True), places=1)
