...
class TestPopulationReporting(unittest.TestCase):
    ...
    def get_event_count(self, pop, outcomeType, wave):
        return pd.Series([person.has_outcome_during_wave(wave, outcomeType) for i, person in pop._people.iteritems()]).sum()
    ...
    def testAllCasesHaveEvent(self):
        ...
        self.assertAlmostEqual(100000, self.pop1.calculate_mean_age_sex_standardized_incidence(OutcomeType.MI)[0], delta=0.01)
        self.assertAlmostEqual(0, self.pop1.calculate_mean_age_sex_standardized_incidence(OutcomeType.STROKE)[0], delta=0.01)
        self.assertAlmostEqual(0, self.pop1.calculate_mean_age_sex_standardized_mortality(), delta=0.01)
