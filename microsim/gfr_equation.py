...
class GFREquation:
    exponentForGenderCr = pd.DataFrame({'female': [True, True, False, False],
                                        'underThreshold' : [True, False, True, False],
                                        'exponent' : [-0.329, -1.209, -0.411, -1.209] })

    constantForRaceGender = pd.DataFrame({'black' : [True, True, False, False], 
    ...
    def get_gfr_for_person(self, person):
        ...
        exponent = GFREquation.exponentForGenderCr.loc[(GFREquation.exponentForGenderCr['female']==(person._gender==NHANESGender.FEMALE)) & (GFREquation.exponentForGenderCr['underThreshold'] == (person._creatinine[-1] <= crThreshold))].iloc[0]['exponent']
        constant = GFREquation.constantForRaceGender.loc[(GFREquation.constantForRaceGender['black']==(person._raceEthnicity==NHANESRaceEthnicity.NON_HISPANIC_BLACK)) & (GFREquation.constantForRaceGender['female'] == (person._gender==NHANESGender.FEMALE))].iloc[0]['constant']
